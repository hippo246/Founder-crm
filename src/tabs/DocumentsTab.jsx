...
export default function DocumentsTab({ documents, setDocuments, addAudit, role, contacts, projects, tags , workspaceId = "workspace-1" , onLinkedSave}) {
  ...
  const save = (f) => {
    if (editing) { const u = (documents||[]).map(d=>d.id===editing.id?{...editing,...f}:d); setDocuments(u); saveWorkspaceData("documents", u, workspaceId); addAudit("Documents","Update",`Updated doc: ${f.name}`); toast("Document updated"); }
    else { const nd = {...f, id: genId()}; const u = [nd,...(documents||[])]; setDocuments(u); saveWorkspaceData("documents", u, workspaceId); addAudit("Documents","Create",`Added doc: ${f.name}`); toast("Document added"); }
      ...
      {filtered.length === 0 ? <EmptyState icon="📄" title="No documents" sub="Track your proposals, contracts, and files." action={<button style={btnStyle("primary")} onClick={() => setShowForm(true)}>+ Add document</button>} /> : (
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead><tr style={{ background: "var(--surface)" }}>{["Name","Type","Client","Project","Status","",""].map(h=><th key={h} style={{ padding: "10px 14px", textAlign: "left", fontWeight: 600, color: "var(--text-muted)", fontSize: 11, textTransform: "uppercase", whiteSpace: "nowrap" }}>{h}</th>)}</tr></thead>
